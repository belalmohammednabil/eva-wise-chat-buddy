...
const EvaChatbot: React.FC<ChatbotProps> = ({ apiKey = 'demo-key' }) => {
  ...
  const [userTonePreference, setUserTonePreference] = useState<'formal' | 'informal'>('informal');
    ...
    if (detectedProblem) {
      const recommendations = {
        acne: {
          ...
          arMessage: `${userTonePreference === 'formal' ? 'أتفهم مشكلتك مع الحبوب، وهذا أمر شائع يمكن علاجه' : 'فهمت إن عندك مشكلة حبوب! 🤗 دي مشكلة شائعة وليها حل'}:\n\n🧴 المنتجات المناسبة:\n• غسول إيفا اللطيف للبشرة الدهنية - 150ج\n  ▫️ يحتوي على حمض الساليسيليك 2% لتنظيف المسام\n  ▫️ الزنك PCA يقلل البكتيريا\n  ▫️ الألوة فيرا تهدئ الالتهاب\n\n• سيرم فيتامين C المضاد للأكسدة - 350ج\n  ▫️ يفتح البقع الداكنة من آثار الحبوب\n  ▫️ يحارب البكتيريا الضارة\n\n${userTonePreference === 'formal' ? '🔄 البرنامج العلاجي الموصى به:' : '🔄 الروتين المثالي:'}\nصباحاً: غسول → سيرم → مرطب خفيف → واقي شمس\nمساءً: غسول → علاج موضعي → مرطب مهدئ\n\n⚠️ ${userTonePreference === 'formal' ? 'إرشادات طبية مهمة:' : 'نصائح مهمة:'}\n• ${userTonePreference === 'formal' ? 'يُنصح بعدم عصر الحبوب نهائياً' : 'لا تعصر الحبوب نهائياً'}\n• ${userTonePreference === 'formal' ? 'يُفضل تغيير غطاء الوسادة يومياً' : 'غير غطاء الوسادة يومياً'}\n• ${userTonePreference === 'formal' ? 'تجنب لمس منطقة الوجه' : 'تجنب لمس الوجه'}\n• ${userTonePreference === 'formal' ? 'النتائج تظهر خلال 4-6 أسابيع' : 'النتائج تظهر بعد 4-6 أسابيع'}\n\n${userTonePreference === 'formal' ? 'هل تود معرفة المزيد عن منتج محدد؟' : 'عايز تعرف أكتر عن منتج معين؟'}`,
          enMessage: `${userTonePreference === 'formal' ? 'I understand your acne concerns. This is a common condition that can be effectively treated' : 'I understand you have acne concerns! 🤗 This is common and treatable'}:\n\n🧴 Recommended Products:\n• Eva Gentle Facial Cleanser for Oily Skin - 150 EGP\n  ▫️ Contains 2% Salicylic Acid for pore cleansing\n  ▫️ Zinc PCA reduces bacteria\n  ▫️ Aloe Vera soothes inflammation\n\n• Vitamin C Antioxidant Serum - 350 EGP\n  ▫️ Brightens dark spots from acne marks\n  ▫️ Fights harmful bacteria\n\n${userTonePreference === 'formal' ? '🔄 Recommended Treatment Protocol:' : '🔄 Perfect Routine:'}\nMorning: Cleanser → Serum → Light moisturizer → Sunscreen\nEvening: Cleanser → Spot treatment → Soothing moisturizer\n\n⚠️ ${userTonePreference === 'formal' ? 'Important Medical Guidelines:' : 'Important Tips:'}\n• ${userTonePreference === 'formal' ? 'It is strongly advised not to squeeze pimples' : 'Never squeeze pimples'}\n• ${userTonePreference === 'formal' ? 'Daily pillowcase changes are recommended' : 'Change pillowcase daily'}\n• ${userTonePreference === 'formal' ? 'Avoid touching the facial area' : 'Avoid touching face'}\n• ${userTonePreference === 'formal' ? 'Results typically appear within 4-6 weeks' : 'Results show after 4-6 weeks'}\n\n${userTonePreference === 'formal' ? 'Would you like to know more about a specific product?' : 'Want to know more about a specific product?'}`
        },
        dryness: {
          products: ['مرطب إيفا المائي للبشرة الجافة', 'مقشر إيفا اللطيف للبشرة الحساسة'],
          arMessage: `${userTonePreference === 'formal' ? 'ألاحظ أن بشرتك تعاني من الجفاف، وهذا أمر قابل للعلاج بالمنتجات المناسبة' : 'أشوف إن بشرتك جافة! 💧 مش مشكلة، إيفا عندها الحل'}:\n\n🧴 المنتجات المناسبة:\n• مرطب إيفا المائي للبشرة الجافة - 220ج\n  ▫️ حمض الهيالورونيك يحتفظ بالرطوبة 48 ساعة\n  ▫️ نياسيناميد 5% يقوي حاجز البشرة\n  ▫️ السيراميدز تمنع فقدان الماء\n\n• مقشر إيفا اللطيف للبشرة الحساسة - 180ج\n  ▫️ أحماض فواكه طبيعية تزيل الجلد الميت\n  ▫️ الشوفان والعسل يرطبان بعمق\n\n${userTonePreference === 'formal' ? '🔄 البرنامج العلاجي الموصى به:' : '🔄 الروتين المثالي:'}\nصباحاً: غسول لطيف → سيرم مرطب → مرطب غني → واقي شمس\nمساءً: زيت منظف → سيرم → كريم ليلي مكثف\nأسبوعياً: مقشر لطيف مرة واحدة\n\n⚠️ ${userTonePreference === 'formal' ? 'إرشادات العناية الأساسية:' : 'نصائح ذهبية:'}\n• ${userTonePreference === 'formal' ? 'يُنصح بتجنب الماء الساخن' : 'تجنب الماء الساخن'}\n• ${userTonePreference === 'formal' ? 'استخدام المرطب على البشرة الرطبة' : 'استخدم مرطب على البشرة الرطبة'}\n• ${userTonePreference === 'formal' ? 'شرب 2 لتر من الماء يومياً' : 'اشرب 2 لتر ماء يومياً'}\n• ${userTonePreference === 'formal' ? 'استخدام مرطب الجو في فصل الشتاء' : 'استخدم مرطب الجو في الشتا'}\n\n${userTonePreference === 'formal' ? 'هل تود معرفة المزيد؟' : 'عايز تعرف أكتر؟'}`,
          enMessage: `${userTonePreference === 'formal' ? 'I observe that your skin is experiencing dryness, which can be treated with appropriate products' : 'I see your skin is dry! 💧 No problem, Eva has the solution'}:\n\n🧴 Recommended Products:\n• Eva Hydrating Moisturizer for Dry Skin - 220 EGP\n  ▫️ Hyaluronic Acid retains moisture for 48 hours\n  ▫️ 5% Niacinamide strengthens skin barrier\n  ▫️ Ceramides prevent water loss\n\n• Eva Gentle Exfoliating Scrub - 180 EGP\n  ▫️ Natural fruit acids remove dead skin\n  ▫️ Oats and honey deeply moisturize\n\n${userTonePreference === 'formal' ? '🔄 Recommended Treatment Protocol:' : '🔄 Perfect Routine:'}\nMorning: Gentle cleanser → Hydrating serum → Rich moisturizer → Sunscreen\nEvening: Oil cleanser → Serum → Intensive night cream\nWeekly: Gentle scrub once\n\n⚠️ ${userTonePreference === 'formal' ? 'Essential Care Guidelines:' : 'Golden Tips:'}\n• ${userTonePreference === 'formal' ? 'It is recommended to avoid hot water' : 'Avoid hot water'}\n• ${userTonePreference === 'formal' ? 'Apply moisturizer on damp skin' : 'Apply moisturizer on damp skin'}\n• ${userTonePreference === 'formal' ? 'Consume 2L of water daily' : 'Drink 2L water daily'}\n• ${userTonePreference === 'formal' ? 'Use a humidifier during winter' : 'Use humidifier in winter'}\n\n${userTonePreference === 'formal' ? 'Would you like to know more?' : 'Want to know more?'}`
        },
        oily: {
          products: ['غسول إيفا اللطيف للبشرة الدهنية', 'واقي الشمس إيفا SPF 50+'],
          arMessage: `${userTonePreference === 'formal' ? 'البشرة الدهنية لها مميزات عديدة عند العناية الصحيحة بها' : 'بشرتك دهنية؟ 🌟 ده مش عيب، ده نعمة لو عرفتِ تتعاملي معاها صح'}:\n\n🧴 المنتجات المناسبة:\n• غسول إيفا اللطيف للبشرة الدهنية - 150ج\n  ▫️ ينظف الزيوت الزائدة بدون جفاف\n  ▫️ حمض الساليسيليك ينظف المسام بعمق\n\n• واقي الشمس إيفا SPF 50+ - 280ج\n  ▫️ تركيبة خفيفة غير دهنية\n  ▫️ مقاوم للماء والعرق\n  ▫️ لا يسد المسام\n\n${userTonePreference === 'formal' ? '🔄 البرنامج العلاجي الموصى به:' : '🔄 الروتين المثالي:'}\nصباحاً: غسول → تونر قابض → مرطب خفيف → واقي شمس\nمساءً: غسول عميق → تونر → سيرم نياسيناميد → مرطب ليلي\n\n💡 ${userTonePreference === 'formal' ? 'معلومات علمية مهمة:' : 'حقائق مهمة:'}\n• ${userTonePreference === 'formal' ? 'البشرة الدهنية تظهر علامات التقدم في السن بشكل أبطأ' : 'البشرة الدهنية تتقدم في السن أبطأ'}\n• ${userTonePreference === 'formal' ? 'الترطيب ضروري حتى للبشرة الدهنية' : 'الترطيب ضروري حتى للبشرة الدهنية'}\n• ${userTonePreference === 'formal' ? 'يُنصح بتجنب الإفراط في التنظيف' : 'تجنب الإفراط في التنظيف'}\n• ${userTonePreference === 'formal' ? 'شرب الماء وتقليل السكريات يساعد' : 'اشرب الماء وقلل السكريات'}\n\n${userTonePreference === 'formal' ? 'هل تريد المزيد من الإرشادات؟' : 'عايز نصايح أكتر؟'}`,
          enMessage: `${userTonePreference === 'formal' ? 'Oily skin has many advantages when properly cared for' : 'Oily skin? 🌟 That\'s not a flaw, it\'s a blessing if you handle it right'}:\n\n🧴 Recommended Products:\n• Eva Gentle Cleanser for Oily Skin - 150 EGP\n  ▫️ Removes excess oil without drying\n  ▫️ Salicylic acid deep cleans pores\n\n• Eva Sunscreen SPF 50+ - 280 EGP\n  ▫️ Lightweight non-greasy formula\n  ▫️ Water and sweat resistant\n  ▫️ Non-comedogenic\n\n${userTonePreference === 'formal' ? '🔄 Recommended Treatment Protocol:' : '🔄 Perfect Routine:'}\nMorning: Cleanser → Astringent toner → Light moisturizer → Sunscreen\nEvening: Deep cleanser → Toner → Niacinamide serum → Night moisturizer\n\n💡 ${userTonePreference === 'formal' ? 'Important Scientific Information:' : 'Important Facts:'}\n• ${userTonePreference === 'formal' ? 'Oily skin shows signs of aging more slowly' : 'Oily skin ages slower'}\n• ${userTonePreference === 'formal' ? 'Moisturizing is essential even for oily skin' : 'Moisturizing is essential even for oily skin'}\n• ${userTonePreference === 'formal' ? 'It is recommended to avoid over-cleansing' : 'Avoid over-cleansing'}\n• ${userTonePreference === 'formal' ? 'Drinking water and reducing sugars helps' : 'Drink water and reduce sugars'}\n\n${userTonePreference === 'formal' ? 'Would you like more guidance?' : 'Want more tips?'}`
        }
      };
      ...
      <div className="bg-chat-surface border-b border-chat-border p-4 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <Select value={userTonePreference} onValueChange={(value: 'formal' | 'informal') => setUserTonePreference(value)}>
              <SelectTrigger className="w-28 h-8 text-xs bg-chat-card border-chat-border">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="informal">{language === 'ar' ? 'ودود' : 'Friendly'}</SelectItem>
